...
!> MODULE :   Solution of chapter 10 exercises
!! 
!! @author C. Airiau
!!
...
write(6,110) 'Exercice 10.1 : normal shock'
...
write(6,*) 'Ti0                     = ',Ti0
write(6,*) 'Pi0                     = ',Pi0
write(6,*) 'rhoi0                   = ',rhoi0, Pi0/(r*Ti0)
write(6,*) 'a0                      = ',a0
write(6,*) 'U0                      = ',U0

write(6,100) 'Normal shock downstream state'
...
delta_s=entropy_variation(Pi0,Pi1)
write(6,*) 'M1                      = ',M1
write(6,*) 'T1                      = ',T1
write(6,*) 'P1                      = ',P1
write(6,*) 'rho1                    = ',rho1
...
!******************************************************************
!>@details Practical calculus of a normal shock wave
...
write(6,110) 'Exercice 10.2 : Practical calculus of a normal shock wave'
Cp=gam*r/(gam-1.d0)
...
write(6,100) 'Isentropic flow from U0 to U1'
...
!******************************************************************
subroutine Exercice_10_10
!******************************************************************
!>@details  Laval nozzle
implicit none

real(kind=8)        :: Rinlet=0.4d0,Rexit=0.12d0,Rcol=0.09d0
real(kind=8)        :: Ti0=350.d0,Pi0,Pa=101325.d0
real(kind=8)        :: Me,omega,section_ratio,Rcrit,Mcol,Scol,Pexit
real(kind=8)        :: Minlet,Tinlet,ainlet,Uinlet,rhoinlet,Pinlet
real(kind=8)        :: Qm,Pi2
real(kind=8)        :: Rchoc,Mchoc,Mdownstr

if (fichier) open(6,form='formatted',file='Solution_10-10.out')
write(6,110) 'Laval nozzle'
...
Pi0=Pa/omega;   
write(6,*) 'Pi_0                    = ',Pi0
section_ratio= A_Acritic(Me)
Rcrit= Rexit/sqrt(section_ratio)
write(6,*) 'A/Ac                    = ',section_ratio
write(6,*) 'R_crit                  = ',Rcrit

write(6,*) 'Col :'
section_ratio=(Rcol/Rcrit)**2
Mcol=inverse_A_Acritic(0.5d0,section_ratio,1,1,45,1.d-12) 
write(6,*) 'Acol/Ac                 = ',section_ratio
write(6,*) ' Mcol                   = ',Mcol

write(6,*) 'Inlet :'

section_ratio=(Rinlet/Rcrit)**2
Minlet=inverse_A_Acritic(0.5d0,section_ratio,1,1,45,1.d-12) 
write(6,*) 'Ainlet/Ac               = ',section_ratio
...
write(6,*) 'Mass flow rate Qm       = ',Qm

write(6,100) 'Question 2-3'
Rcrit=Rcol
section_ratio=(Rexit/Rcrit)**2
Me=inverse_A_Acritic(0.5d0,section_ratio,1,1,45,1.d-12) 
omega=P_Pi(Me); 
write(6,*) 'P/Pi                    =',omega
Pi0=Pa/omega;   
write(6,*) 'Pi_O                    =',Pi0
...
write(6,*) ' if Pi > Pi0 previous cas and/or '
write(6,*) 'Pexit > Pa'

write(6,100) 'Question 4-2'

Me=2.d0  ! to get P2/P1 = 4.5
!vérification
write(6,*) 'Me                      = ',Me
write(6,*) 'P2/P1  across the shock = ',P2_P1(Me)
Pexit=Pa/P2_P1(Me)
...
write(6,*) 'Exit Mach before the shock  = ', Me
write(6,*) 'Exirt Mach after the shock  = ', Mach_downstr(Me)


write(6,100) 'Question 4-4'
write(6,*) 'Pi0 is decreased of 20%'
Pi0=0.80d0*Pi0
Rchoc=Rcol+ 0.90d0*(Rexit-Rcol)
write(6,*)  'assuming that the shock is located in the section of radius :  ',Rchoc
section_ratio=(Rchoc/Rcol)**2
Mchoc=inverse_A_Acritic(1.5d0,section_ratio,1,1,45,1.d-12) 
write(6,*) 'Upstream Mach              = ', Mchoc
Mdownstr=Mach_downstr(Mchoc)
write(6,*) 'Mach downstream            = ', Mdownstr
Rcrit=Rchoc/A_Acritic(Mdownstr)
write(6,*) 'New critical section       = ', Rcrit
section_ratio=(Rexit/Rcrit)**2
Me=inverse_A_Acritic(0.5d0,section_ratio,1,1,45,1.d-12) 
...
write(6,*) 'Pi after the shock     = ',Pi2
Pexit=Pi2*P_Pi(Me)
write(6,*) 'Pexit                  = ', Pexit
write(6,*) 'Pexit /Pa              = ', Pexit/Pa

if (Pexit/Pa .ge.1.d0) then 
        write(*,*) 'You must increase Rchoc'
        write(*,*) 'the shock should be more downtream,  to the exit'
else 
        write(*,*) 'you must decrease Rchoc'
        write(*,*) 'the shock should be more upstream,  to the throat'
end if

100  format(/,50('#'),/,3x,a,/,50('#'),/)
110  format(/,50('*'),/,3x,a,/,50('*'),/)
if (fichier) close(6)
end subroutine Exercice_10_10


!******************************************************************
subroutine Exercice_10_3
!******************************************************************
!>@details Subsonic Fanno flow
...
real(kind=8)                ::  height,width,S,c
real(kind=8)                ::  L1crit,fM1,fM1_new,M1_new

if (fichier) open(6,form='formatted',file='Solution_10-3.out')
write(6,110) 'Exercice Subsonic Fanno flow 10.3'

!************************************************
write(6,100) 'Subsonic Fanno flow with disk section'
!************************************************
...
F2=F1-RHS
if (Lcrit.gt. L) then
    write(6,*) 'fully subsonic flow'
    M2=inverse_Fanno(0.1d0,F2)
    write(6,*) 'F(M2)                   = ',F2
    write(6,*) 'M2                      = ',M2
else
    write(6,*) 'Lcrit < L :  ==> new calculs to perform'
end if
...
!************************************************
write(6,100) 'Subsonic Fanno flow with rectangular sectio'
!************************************************
M1=0.5d0
! if rectangular section
width=0.02d0;height=0.01d0;
S=width*height;c=2.d0*(width+height)
Dh=4.d0*S/c
 ! if disk section 
Dh=0.2d0
write(6,*) 'M1                      = ',M1
...
!************************************************
write(6,110) 'Supersonic shocked Fanno flow , exercice 10.4'
!************************************************
Dh=1.d0; L=30.d0*Dh; 
M1=3.0d0
write(6,*) 'M1                      = ',M1
...
write(6,*) 'L1 critical             = ',L1crit
if (L1crit.gt. L) then
    write(6,*) 'flow fully supersonic without shock'
    stop
else
    write(6,*) 'Lcrit < L :  ==> supersonic flow with shock'
end if
...
L2=L-L1;
M2=Mach_downstr(Mx);
write(6,*) 'Mach downstream          = ',M2
L2crit=Sonic_Length(M2)*Dh/(4.d0*cf)
write(6,*) 'L2 critical             = ',L2crit
...
!******************************************************************
!>@details Sub and supersonic Rayleigh flow 
...
!************************************************
write(6,100) 'Rayleigh flow, exercice 10.5'
!************************************************


P0=1.d5         ! inlet pressure in  Pa 
T0=300.d0       ! inlet temperature
M0=2.d0         ! inlet Mach number
Cp=gam*r/(gam-1.d0)
...
!================================
!>@details Rayleigh flow for a given Mach number
implicit none
real(kind=8),intent(in) :: M0
real(kind=8)            :: M_init,M0prime,rp0,P0prime,T0prime,rT0


write(6,110) 'Q1: heat flux qmax'
...
write(6,*) 'M upstream               = ',M0
write(6,*) 'P upstream               = ',P0
write(6,*) 'T upstream               = ',T0
write(6,*) 'T0/Ti0                   = ',rTi0
write(6,*) 'Ti0                      = ',Ti0
write(6,*) 'Ra0                      = ',Ra0
...
if (M0.gt.1.d0) then
    write(6,110) 'Q3: qmax=2 qmax, supersonic'
    M_init=2d0
else
    write(6,110) 'Q3: qmax=2 qmax, subsonic'
    M_init=0.5d0
end if
qmax=2.d0*qmax
rTic=1.d0+qmax/hi0
...
write(6,*) 'Ra0                      = ',Ra0
write(6,*) 'M upstream (new)         = ',M0prime
...
!******************************************************************
!>@details Rayleigh flow + combustion
...
!************************************************
write(6,100) 'Rayleigh flow + combustion, exercice 10.6'
!************************************************

u0=80.d0         ! inlet velocity
P0=1.4d4         ! inlet pressure in Pa 
T0=290.d0        ! inlet temperature
q=800.d3         ! heat added flux  q
Cp=gam*r/(gam-1.d0)
S0=1.d0
...
rP=(1.d0+gam*M0**2)/(gam+1.d0)  ! P1/P0 (critical) 
ru=rP*(M1/M0)**2
rT=(M1/M0 *rP)**2
P1=rP*P0
rho1=P1/(r*T1) 
...
write(6,*) 'velocity verification    = ',u1/u0,ru,rho0/rho1
...
!******************************************************************
!>@details Pitot tube in supersonic flow
...
!************************************************
write(6,100) 'Pitot tube in supersonic flow, exercice 10.8'
!************************************************
...
M2=Inverse_P_Pi(rP2)
write(6,*) 'Mach M2                 = ',M2
M0=mach_downstr(M2)         ! the inverse relation is equal to itself !
write(6,*) 'Mach M0                 = ',M0
Ti0=Ti2
rT0=T_Ti(M0)
write(6,*) 'T0/Ti0                  = ',rT0
...
!******************************************************************
!>@details Flow regimes in nozzles

real(kind=8)        :: Ti0,Pi0,rPs
real(kind=8)        :: M2,rP2,rPi2,P2,Pi2,Sc2
real(kind=8)        :: Sc,Ss,Ms,Ps,section_ratio,qm
...
!************************************************
write(6,100) 'Flow regimes in nozzles, exercice 10.9'
!************************************************

Pi0=1.d6        ! Pressure in Pa
Ti0=600.d0
Sc=5.d-4        ! critical section in cm^2
Ss=20.d-4       ! exit section in cm^2


section_ratio=Ss/Sc
write(6,110) 'Q1 : Regime C'
Ms= inverse_A_Acritic(2.d0,section_ratio,1,1,45,1.d-12) 
write(6,*) 'Ms                      = ',Ms
rPs=P_Pi(Ms);
write(6,*) 'Ps/Pis                  = ',rPs
Ps=Pi0*rPs
...
write(6,110) 'Q2: mass flow rate qm'
qm=Sonic_Mass_flow (Pi0,Ti0,Sc)
write(6,*) 'qm                      = ',qm


write(6,110) 'Q3: subsonic regime B '
Ms= inverse_A_Acritic(0.5d0,section_ratio,1,1,45,1.d-12)
write(6,*) 'Ms                      = ',Ms
rPs=P_Pi(Ms);
write(6,*) 'Ps/Pis                  = ',rPs
Ps=Pi0*rPs
...
write(6,110) 'Q4: supersonic regime D with exit shock'
Ms= inverse_A_Acritic(2.d0,section_ratio,1,1,45,1.d-12) 
M2=mach_downstr(Ms)
rP2=P_Pi(M2);
rPi2=Pi2_Pi1(Ms);
...
write(6,*) 'Mach upstream            = ',Ms
write(6,*) 'Pressure upstream        = ',P_Pi(Ms)*Pi0
write(6,*) 'Mach downstream          = ',M2
write(6,*) 'P/Pi downstream          = ',rP2
write(6,*) 'P downstr/P upstream     = ',P2_P1(Ms)
write(6,*) 'Pi downstr/Pi upstream   = ',rPi2
write(6,*) 'P exit                   = ',P2
write(6,*) 'Pi exit                  = ',Pi2
 

write(6,110) 'Q5: Régime D prime, supersonic flow in the diverging'
write(6,*) ' enter S1 in cm2'
!read*,S1
S1=S1*1.d-4
S1=12.2d-4
M1= inverse_A_Acritic(1.5d0,S1/Sc,1,1,45,1.d-12)
M2= mach_downstr(M1)
write(6,*) 'Section upstream            = ',S1
write(6,*) 'Mach upstream               = ',M1
write(6,*) 'Mach downstream             = ',M2
rP1=P2_P1(M1)
write(6,*) 'P downstream/P upstream     = ',rP1
section_ratio=A_Acritic(M2)
Sc2=S1/section_ratio
write(6,*) 'New critical section        = ',Sc2
M2= inverse_A_Acritic(0.5d0,Ss/Sc2,1,1,45,1.d-12)
write(6,*) 'Mach exit                   = ',M2
rPi2=Pi2_Pi1(M1);
write(6,*) 'Pi downstream/Pi upstream   = ',rPi2

Pa=5.d5
Ps=P_Pi(M2)*rPi2*Pi0/Pa
write(6,*) ' = 1 ?                   = ',Ps

write(6,110) 'Q5: Pa= 100 000, oblique shock, under expanded nozzle'
P2=1.d5
! we need results from question 1 :
write(6,*) 'reminder of question 1 results :'
section_ratio=Ss/Sc
Ms= inverse_A_Acritic(2.d0,section_ratio,1,1,45,1.d-12) 
write(6,*) 'Ms                      = ',Ms
Ps=Pi0*P_Pi(Ms)
write(6,*) 'Ps                      = ',Ps
rP2=P2/Ps
write(6,*) 'P downstream/P upstream = ',rP2
Mns=Inverse_P2_P1(rP2)
write(6,*) 'Mn exit upstream        = ',Mns
sigma2=asin(Mns/Ms)
write(6,*) 'Shock angle             = ',sigma2/val_pi*180.d0

tmp=(2.d0+(gam-1.d0)*Mns**2)/((gam+1.d0)*Mns**2)*tan(sigma2)
theta=(sigma2-atan(tmp))/val_pi*180.d0
write(6,*) 'Deviation angle (deg)   = ',theta,atan(tmp)/val_pi*180.d0

write(6,110) 'Q6: 20 000 Pa, over expanded nozzle '
P2=2.d4;
rP2=P2/Ps
M2=Inverse_P_Pi(P2/Pi0)
write(6,*) 'Ms                       = ',Ms
write(6,*) 'P downstream/P upstream  = ',rP2
write(6,*) 'M downstream expansion   = ',M2
Om2=omega(M2);Oms=omega(Ms)
deltaTheta=Om2-Oms
write(6,*) 'omega exit  upstream     = ',Oms/val_pi*180.d0
write(6,*) 'omega exit  downstream   = ',Om2/val_pi*180.d0
