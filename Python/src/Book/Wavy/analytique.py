...
Wavy package:
    ...
    * analytical solutions with expansions
    * plots
"""
...
def f_values(y,k):
    """
    calculate vectorial function f
    
    Params:
        * y(real) : normal coordinates
        * k (real) : real wave number (small parameter)
...
def _plot_solution(y, Ysol, Titre="Solution", lim=np.array([0, 5]), scale="linear", display=False):
    """
    plot of the analytical solution of Y, wrt y
    """
    if display:
        t = Ysol.shape
        print("size of the array                    : %i x %i "%(t[0],t[1]))
        N = t[1] 
...
def _plot_u(y, Ysol, Titre="Solution", lim=np.array([0, 5]), scale="linear", display=False):
    """
    plot of the analytical velocity wrt x
    """
    if display:
...
def analytical_solution(N, npt, k0, x, y, display="False"):
    """
    ...
    for  i in range(npt):
        f[i, :] = f_values(y[i], k0)
    _plot_solution(y, f, Titre=r"analytical solution $f$ ", lim=np.array([0, y[-1]]), scale="semilogy", display=display)
    #print("f(0) = " ,f[0,:])

    u = np.zeros(len(x))
    for i, xloc in enumerate(x):
        u[i] = q(xloc, y[0], k0)
    _plot_u(x, u, Titre=r"$|{\bf u}|_{wall}$ (analytical solution)", lim=np.array([ x[0], x[-1] ]), scale="linear", display=display)
    return u,f 
