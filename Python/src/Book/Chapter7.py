...
Correction of the chapter 7 exercises
    ..
"""
...
from IncompressibleFlow.PanelMethod import velocity_field, velocity_field_vector
...
def Exercice7_0():
    ...
    Outputs :
        - calculate t if k or the trailing edge angle for a given thickness are known
        - calculate k if t for a given thickness is known
        - plot in the (t,k) map the thickness value
    """
    height, width =8, 10                # window sizes
    set_title(" Van-Hooren airfoil parameters")
    chord = np.float64(1.)              # chord length
    #eps = 0.1                          # to avoid 0 division at theta=0 or 180 °
    npt = int(360/2+1)                       # number of points on the airfoil
    ...
    #                                     put 0 if k is set, otherwise  is calculated
    thickness_ref= 0.15                 # targeted thickness
    ...
    option = 0  # = 0 : k function of t, Newton method
    #           # = 1 : t function of k, Newton method
    #           # = 2 :  (t, k) map
    
    def Thickness_law(param, option=0):
        """
        Van Hooren airfoil thickness function of k or t
        """
        if option == 0:
            x, y, theta = Van_Hooren_gmtry(chord, t, param, npt, plot_profil, origin=False, dtheta=0.0)
        else:
            x, y, theta = Van_Hooren_gmtry(chord, param, k, npt, plot_profil, origin=False, dtheta=0.0)
        Imax = np.argmax(y)
        return 2*y[Imax],x[Imax]

    def ThicknessTable(t, k):
        """
        Van Hooren airfoil thickness for (t,k) map
        """
    ...
    if option < 2:
        ...
        if error > errorMax:
            ...
            if option == 0:
                ...
                t = par
            x, y, theta = Van_Hooren_gmtry(chord, t, k, npt, plot_profil, origin=False, dtheta=0.0)
            print('Solution for k = %f, t = %f, delta_TE = %f  , thickness= %f,  x_pos  = %1.3f '%(k, t, (k-2)*90,
                                                                                                   thickness, xpos))
            angle = np.rad2deg( np.arctan2(y[n]-y[n-1],x[n]-x[n-1]))
            print('theta = %f, trailing edge  = %f = %f'%(np.rad2deg(theta[n]), angle, (k-2)*90))

    elif option == 2:
        N = 41
        t_min, t_max=0, 0.1
        k_min, k_max=1.7, 2
        t, k = grid_definition(N, t_min, t_max, k_min, k_max)
        ...
        if plot_option == 0:
            ...
            # iso contour de la position du maximal suivant x
            ...
            # to get the iso-line with the reference targeted thickness
            cs = plt.contour(t, k, thickness, [thickness_ref])
            p = cs.collections[0].get_paths()[0]
            v = p.vertices

            plt.plot([w[0] for w in v],[w[1] for w in v], linestyle='-', linewidth=1, color='black')
            plt.xlabel(r't,$\varepsilon$')
            plt.ylabel('k') 
            ...
            print('Interpolation for a thickness of  %f' % thickness_ref)
            print(' t  \t \t k')
            for i in range(N):
...
def Exercice7_1():
    """ 
    Van-Hooren's airfoil, parameter map in the k-t plane (thickness)
    """
    set_title("Van-Hooren airfoil :  analytical solution")
    ...
    x = r*np.cos(phi)+ell
    y = r*np.sin(phi)
    print('max thickness = %f' % (2*np.amax(y)))
    Imax = np.argmax(y)
    print('Imax = %i, x_max = %f' % (Imax, x[Imax]))
    if plot :
        nfig = 1 
        plt.figure(nfig, figsize=(10, 3))
        plt.title('airfoil', fontsize=14, fontweight='bold')
        ...
        if angle_flag:
            ...
            # reference_filepath = os.path.join('Book/Data', 'NACA0015.dat')
    ...
    for alpha in list(aoa):
        ...
        Kp = 1-(U/U0)**2
        print('Kp minimal = %f'%(np.amin(Kp)))
        Cl = 8*np.pi*a/ell*np.sin(Alpha)
        print('alpha = %2.0f, Cl = %1.3f'%(alpha,Cl))
        np.savetxt('Book/Data/Kp_VanVooren_%i_deg.dat'%(alpha), np.c_[x, Kp], fmt='%f', delimiter=' ', newline='\n')
        if plot:
            
            # perturbed velocity plot:
...
def Kp_thick_effect(theta, t):
    """
    Kp of symmetrical thick airfoil
    """    
    b0 = 16/9*np.sqrt(3)*t
    b1 = -b0
...
def Exercice7_2():
    ...
    Parabolic airfoil
    """
    set_title("Parabolic airfoil : analytical solution")

    # data, parameters
    thick_effect = True                  # thickness flag
    # U0 = np.float64(1)                # reference velocity
    # chord = np.float64(1.)            # chord length
    # ell = chord
    aoa = np.array([0., 5.0, 10.0])/180*np.pi # angle of attack table
    ...
    if thick_effect:
        ...
        Kp_thick = Kp_thick_effect(theta, t)
    else:
        ye = 0
        Kp_thick = 0
    n = len(theta)              
...
def Exercice7_3():
    ...
    This code is a translation in python of the original fortran77 code provided by Katz and Plotkins where 
    the circulations are given positive for a positive lift.
    It is the opposite sign with our notations
    """
    chord = 1  # is it correct here ?
    ...
    def point_location_panel(N, x0, dx, distribution):
        ...
        """
        # initialization
        xa = np.zeros(N, dtype=float)
        ya = np.zeros(N, dtype=float)
        xb = np.zeros(N, dtype=float)
        yb = np.zeros(N, dtype=float)
        xc = np.zeros(N, dtype=float)
        yc = np.zeros(N, dtype=float)
        xv = np.zeros(N, dtype=float)
        yv = np.zeros(N, dtype=float)
    ...
    # all the points are different in the panel
...
def Exercice7_4():
    """ 
    Van-Hooren's airfoil, sources distribution, Neumann's approach
    """
    set_title("Van-Hooren airfoil : analytical solution, 2nd version")
    set_title("Point source distribution")
    ...
    """
    methodology  options, calculus and visualization :
    """

    show_control_points     = False             # to have some intermediate plot  set True
    symmetrie               = False             # solve the control point only on the upper wall,
    #                                            otherwise on the full airfoil surface
    ...
    row_source              = 0                 # ou int(np.ceil(npt/3)),
    #                                           this parameter is no longer useful set to 0
    plot_sources            = True              # to plot the source distribution
    improved_control_point  = False             # change the control point at the leading and trailing edge
    #                                             if symmetry is used
    more_details            = False 
    translation_xc          = np.float(0.1)     # translate the first and last source points at the L.E. and T.E.

    # To work well need to set "test_somme_source = True" and not use the symmetry option

    fst = Freestream(U_inf=U0, alpha=aoa,rho=rho0)  # upstream flow
    # Geometry
    x, y, theta = Van_Hooren_gmtry(chord, t, k, npt, plot_profil, dtheta=0)
    
    n = int((npt-1)/2)
    xe, ye = x[0:n+1], y[0:n+1]
    xi, yi = xe, -ye                # upper and lower airfoil walls
    print("length of xe = %i, n= %i " % (len(xe),int(n)))
    u = (0, n)
    print(" [xe_LE, xe_TE], [ye_LE,ye_TE] ", xe[list(u)], ye[list(u)])
    ...
    if symmetrie:
        # create panels
        print("Symmetry is used, upper wall calculation only:")
        N = len(xe)-1
        print('Panel numbers = %i' % (N))
        ...
        print("No symmetry assumption, upper and lower wall calculation")
        # create panels
        N = len(x)-1
        print('Panel number = %i' % N)
        panels = np.zeros(N, dtype=object)
        for i in range(N):
            panels[i] = Panel(x[i], y[i], x[i+1], y[i+1])
            if N < N_display:
                print('i = %3i, delta = %3.3f °, x = %f ' % (i, np.rad2deg(panels[i].delta), panels[i].xc))
    ...
    if show_control_points:
        ...
        plt.grid()
        plt.title("Control (collocation) point on the panels")
    ...
    # source location on the main axis
    ...
    # The row of the A matrix set the summation of the sources = 1  
    # idem for the last  RHS component
    ...
    xp = np.zeros((N), dtype=float)        # location of the panel center  
    ...
    if zero_source_sum:
        RHS[row_source] = 0
        A[row_source,:] = 1
        print('Setting sum sigma_k = 0 on the row %i, @ x = %f' % (row_source, xs[row_source]))
    if Ns == N:
        print('Direct matrix inversion')
        S = np.linalg.solve(A, RHS)
    else:
        print('Least square approach')
        S, res, rank, s = np.linalg.lstsq(A, RHS, rcond=-1)

    if more_details:
        print('source vector : S =', S)
    
    print("Sum_k S_k           = %3.10f" % (np.sum(S)))
    error = np.dot(A, S)-RHS
    print("Solution error (system inversion) N_2 = %12.10e" % (np.sqrt(np.dot(error, error))))
    if plot_sources:
    ...
    Ut = np.dot(B,S)+RHSt    # tangential velocity on the collocation points
    Kp = 1-Ut**2             # Pressure coefficient
    for i,panel in enumerate(panels):
        panel.Kp, panel.Ut = Kp[i], Ut[i]
    
    if plot_Kp:
        # load geometry from data file
        reference_filepath = os.path.join('Book/Data', 'Constant_S_Source.dat')
        with open(reference_filepath, 'r') as infile:
            xKatz, CpKatz, ind = np.loadtxt(infile, dtype=float, unpack=True, skiprows=2)
        filepath_ref = os.path.join('Book/Data', 'Kp_VanVooren_0_deg.dat')
        ...
        plt.plot(xp, Kp, linestyle='-', linewidth=1,  color='#CD2305', label='Point source distribution')
        plt.plot((xKatz+1)/2, CpKatz, linestyle='', linewidth=1, marker='o', markersize=4, markevery=2, color='black',
                 label='Uniform distribution  (Katz)')
        plt.plot(xref, Cpref, linestyle='-', linewidth=1, color='blue', label='Analytical ref.')
        plt.legend()
        ech = 0.05
        plt.xlim(0-ech, chord+ech)
        plt.ylim(-1.1, 1.1)
    ...
    if plot_streamlines:
        X, Y = grid_definition(100, -0.2, 1.2, -0.25, 0.25)
...
def Exercice7_10():
    """
    plot of a NACA 4 digits Airfoil
    """
