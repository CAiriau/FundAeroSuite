...
Correction of the chapter 15 exercises : Turbulent flow (without Turbulence class)
    ..
    
It is an old version therefore the translation in english is not performed.
"""
import os
import numpy as np
...
def mixing_length(yp, Rtau, yp_scv):
    """
    choice du modèle de longeur de mélange
    """
    c, Aplus = 0.085, 26.0
    k = Rtau * c
    L = k * np.tanh(kappa * yp / k)
    if turbulence_model == 3:
        L = L * (1 - np.exp(-yp / Aplus))
    elif turbulence_model == 0:
        return kappa * yp
    elif (turbulence_model == 1) and (yp <= yp_scv):
        L = kappa * yp
    return L
...
def dudy(u, y, v0, Rtau, yp_scv):
    ...
    du+/dy+=
    """
    L2 = mixing_length(y, Rtau, yp_scv) ** 2
...
def profil_turbulent_aspiration(Cf, Rtau, Re_v0, yp0, yp_scv, n):
    ...
    print('Modele de longueur de mélange : ', turbulence_model)
...
def calcul_limite_sous_couche_visqueuse(v0):
    ...
    def s2(v0):
        """
        Dérivée continue, version DL order 1 de la fonction f1  
        """
...
def Exercice15_10():
    ...
    Un_yv = False           # choice de l'utilisateur sinon il y a une liste
    Re_tau = 1000
    ...
    eta = np.linspace(0, 6, nl)  # l'épaisseur est de l'order de 6 unités y/delta
    if plot_laminaire:
        fig = plt.figure()
    ...
    if plot_ssv:
        fig = plt.figure()
        fig.suptitle(r'Ecoulement turbulent, sous-couche visqueuse, $v_0^+$ = %1.4f' % (v_0_plus), fontsize=14,
                     fontweight='bold')
        ax = fig.add_subplot(111)
        fig.subplots_adjust(top=0.80)
    ...
    if plot_i:
        fig = plt.figure()
        fig.suptitle(r'Ecoulement turbulent, couche interne, $v_0^+$ = %1.4f' % v_0_plus, fontsize=14,
                     fontweight='bold')
        ax = fig.add_subplot(111)
        fig.subplots_adjust(top=0.80)
        ax.set_xlabel(r'$y^+$', fontsize=20)
    ...
    uref, yref = log_law(Yref, 1 / kappa, 5.28)
    turbulence_model = 3
    ...
    # v0+=1e-6
    for k in range(Ncas):
        turbulence_model = leg[k]
        u_tmp, y_plus_tmp, u_log, y_log, v0Plus[k] = profil_turbulent_aspiration(Cf, Re_tau, Re[k], yp_0, yp_limite, nt)
        for i in range(nt):
            u_t[k, i], y_plus_t[k, i], L[k, i] = u_tmp[i], y_plus_tmp[i], mixing_length(Yref[i], Re_tau, yp_limite)
    ...
    if plot_sol_exacte:
        ...
        for k in range(Ncas - 2):
            ax.semilogx(y_plus_t[k, :], u_t[k, :], '-', linewidth=2,
                        label=r'$v_0^+$ = %1.4f, mod. n° %i' % (v0Plus[k], leg[k]))
        ax.semilogx(y_plus_t[Ncas - 2, :], u_t[Ncas - 2, :], 's', linewidth=2, markevery=(0, 5),
                    label=r'$v_0^+$ = %1.4f, mod. n° %i' % (v0Plus[Ncas - 2], leg[Ncas - 2]))
        ax.semilogx(y_plus_t[Ncas - 1, :], u_t[Ncas - 1, :], '-o', linewidth=2, markevery=(0, 10),
                    label=r'$v_0^+$ = %1.4f, mod. n° %i' % (v0Plus[Ncas - 1], leg[Ncas - 1]))
        ...
        if plot_ssv_i:
            ax.semilogx(y_plus, u_plus, '-', linewidth=3, color='blue',
                        label=r'sous-couche, $v_0^+$ %1.4f' % v_0_plus)
            ax.semilogx(y_plus_i, u_plus_i, '-', linewidth=3, color='orange',
                        label=r'interne, $v_0^+$ %1.4f' % v_0_plus)
    ...
    if plot_Lm:
        ...
        ax.plot(Yref[:], L[Ncas - 2, :], '-s', linewidth=2, markevery=(0, 10),
                label=r'$v_0^+$ %1.4f, mod. n° %i' % (v0Plus[Ncas - 2], leg[Ncas - 2]))
        ax.plot(Yref[:], L[Ncas - 1, :], '-o', linewidth=2, markevery=(0, 0.1),
                label=r'$v_0^+$ = %1.4f, mod. n° %i' % (v0Plus[Ncas - 1], leg[Ncas - 1]))
        # ax.semilogx(Yref[:],L[k,:],'s',linewidth=2,markevery=(0,5),label='Modele v0+ plot mod. n° %i'%(v0Plus[
        # Ncas-2],leg[Ncas-2])) ax.semilogx(Yref[:],L[k,:],'-o',linewidth=2,markevery=(0,10),label='Modele v0+ =
        # %1.4f, mod. n° %i'%(v0Plus[Ncas-1],leg[Ncas-1]))
        ax.axis([0, 200, 0, 100])
        ax.legend(loc='upper left')
    plt.show()
