...
"""
functions for the panel method
"""
...
class Panel:
    ...
    def __init__(self, xa, ya, xb, yb,coef=0.5,n_interior=True,delta_is_theta=False):
        ...
        delta:     slope of the panel ( pi/2< delta < pi/2)
        theta:     angle of the panel ( pi  < delta < pi  )
        """
        ...
        # orientation of panel (angle between x-axis and panel's tangente)
        ...
        self.epsilon=0
        if xb-xa >= 0 :
            self.loc = 'intrados' # upper surface
            self.epsilon=-1
        else:
            self.loc = 'extrados' # lower surface
            self.epsilon=1
...
def arctan3(sint,cost):
    """ 
        Calcul d'un angle par une arctangente continu (A vérifier cependant)
    """
    if sint == 0:
        angle=np.pi/2
    else:
...
def Van_Hooren_gmtry(chord,epsilon,k,npt,plot_option,origin=False,dtheta=0):
    """
    Geométrie du profil de Van-Hooren
    chord  : corde du profil
    npt    : nombre de point 16, 31, 61, 91, 181, 361, etc ...
    k      : paramètre du profil
    epsilon: paramètre d'épaisseur du profil
    origin : permet de choisir si le premier point est au bord d'attaque ou au bord de fuite
    dtheta : angle en degrés pour éviter que le bord d'attaque soit un point de calcul
    """
    ...
    eta=1-k+epsilon*k                          # parameter for perturbed velocity and Kp
    ...
    for i in range(n):
        ...
        if theta[i]==0: 
            ...
            if np.abs(cost-1)<= 1e-13:
                # print('erreur sur theta1 pour i= %i, theta= %f'%(i,theta[i]))
                theta1[i]=0
            else:
    ...
    if plot_option :
        print('Géométrie du profil de Van de Vooren')
...
def definition_grille(N,xmin,xmax,ymin,ymax):        
    """
    Grille avec des points équirépartis
    """
    X, Y = np.meshgrid(np.linspace(xmin,xmax,N), np.linspace(ymin,ymax,N))
    return X,Y

def champ_vitesse(uinf,vinf,X,Y,xs,S,plot_option,xp,yp):
    """
    Champ des vitesses  et ligne de courant sur une grille connaissant les sources sur l'axe
    Potentiel associé à des sources ponctuelles
    """
    Nx,Ny=X.shape
...
def champ_vitesse_vecteur(uinf,vinf,X,Y,xs,S,plot_option,xp,yp,plt_show=False):
    """
    Champ des vitesses sur une grille connaissant les sources sur l'axe
    dessin des vecteurs vitesses sur les points de contrôle des panneaux
    """
    Nx=X.shape;print('Nx = %i'%(Nx))
    Ns=len(xs)
    eps=1.e-5
    ...
    if plot_option:
        ...
        plt.xlabel('x', fontsize=16);plt.ylabel('y', fontsize=16)
        plt.xlim(x_start,x_end);plt.ylim(y_start,y_end)
        plt.quiver(X, Y, u, v, angles='xy',scale=5,width=0.003)  # vecteur vitesse
        plt.scatter(xs, np.zeros((Ns), dtype=float),color='#CD2305', s=80, marker='o', linewidth=0);                  # position des sources
        plt.scatter(X, Y,color='green', s=2, marker='o', linewidth=1);          # position des points de contrôle 
        plt.plot(xp,yp,'r-')
...
def plot_profile_with_panels(panels):
    """
    tracé des panneaux avec les points de contrôle
    """
   
    print(len(panels))
    chord=panels[len(panels)-2].xb
   
    print('longeur dans plot panel = ',len(panels))  
    ...
    plt.plot([panel.xc for panel in panels],[panel.yc for panel in panels],
                 linestyle='', linewidth=0, marker='s', markersize=5, color='blue',label='Point de contrôle')
    plt.plot(np.append([panel.xa for panel in panels], panels[0].xa),
             np.append([panel.ya for panel in panels], panels[0].ya),
             linestyle='-', linewidth=1, marker='o', markersize=6, color='#CD2305',label='Point des panneaux')
...
def plot_source_panels(panels,plt_show=False):
    """
    plot of the source from the panels
    """
    ...
    plt.title('Sources')
    plt.plot([panel.xc for panel in panels],[panel.sigma for panel in panels],linestyle='-', linewidth=1, marker='o', markersize=6, color='#CD2305')
    plt.plot(xc[0:n],sigma[0:n],linestyle='-', linewidth=1, marker='o', markersize=6, color='red',label='extrados')
    plt.plot(xc[n:N],sigma[n:N],linestyle='-', linewidth=1, marker='o', markersize=6, color='blue',label='intrados')
    ech=0.05
    plt.xlim(0-ech, chord+ech)
...
def plot_doublet_panels(panels,plt_show=False):
    ...
    print('n_extrados = ',n) 
    N=len(panels)
    plt.plot([panel.xc for panel in panels],[panel.mu for panel in panels],linestyle='-', linewidth=1, marker='o', markersize=6, color='#CD2305')
    plt.plot(xc[0:n],mu[0:n],linestyle='-', linewidth=1, marker='o', markersize=6, color='red',label='extrados')
    plt.plot(xc[n:N],mu[n:N],linestyle='-', linewidth=1, marker='o', markersize=6, color='blue',label='intrados')
    ech=0.05
    plt.xlim(0-ech, chord+ech)
    plt.ylim(ymin-0.01, ymax+0.01);
    plt.legend()
    ...
    for i in range(len(mu)):
        S+=mu[i]
    print('somme(mu) sur les panneaux  :',S)
...
def plot_doublet_derivative_panels(panels,plt_show=False):
    ...
    for i in range(len(dmudx)):
        if np.isinf(np.abs(dmudx[i])):
            dmudx[i]=0
            print('dmudx infini à %i'%(i))
    xb=[panel.xb for panel in panels]
    width = 10;ech=0.05
    plt.figure(figsize=(width, 0.8*width))
    plt.xlabel('x', fontsize=16); plt.ylabel(r'$d\mu/dx$', fontsize=16)
    plt.title('Doublet derivative')
    n=-1
    for i in range(len(panels)):
        if panels[i].epsilon == 1:
            n+=1
    N=len(panels);print('n_extrados = %i, N = %i'%(n,N)) 
    ymax=np.max(np.abs(dmudx[2:N-2]))
    print('ymax = ',ymax)
    plt.plot(xb[0:n+1],dmudx[0:n+1],linestyle='-', linewidth=1, marker='o', markersize=6, color='red',label='extrados')
    plt.plot(xb[n+1:N-1],dmudx[n+1:N-1],linestyle='-', linewidth=1, marker='o', markersize=6, color='blue',label='intrados')
    plt.xlim(0-ech, chord+ech)
    #plt.ylim(-ymax, ymax);
    plt.legend();plt.grid()
    if plt_show: plt.show()
...
def plot_profile_directions(panels,plt_show=False):
    """
    dessin du profil avec les directions normales et tangentielles sur les panneaux
    """
    x =[panel.xc for panel in panels]
    y =[panel.yc for panel in panels]
    x_start,x_end=-0.6, 1.5
    y_start,y_end=-0.5, 0.5
    size = 10        # taille de la figure
    s=0.5            # longueur des vecteurs
    plt.figure(figsize=(size, (y_end-y_start)/(x_end-x_start)*size))
    plt.xlabel('x', fontsize=16);plt.ylabel('y', fontsize=16)
    plt.xlim(x_start,x_end);plt.ylim(y_start,y_end)
    plt.quiver(x, y,[panel.tx*s for panel in panels],[panel.ty*s for panel in panels], angles='xy',scale=5,width=0.003,color='red')  
    plt.quiver(x, y,[panel.nx*s for panel in panels],[panel.ny*s for panel in panels], angles='xy',scale=5,width=0.003,color='blue')  
    plt.scatter(x, y,color='green', s=2, marker='o', linewidth=1);          # position des points de contrôle 
    plt.plot([panel.xa for panel in panels],[panel.ya for panel in panels],'k-')
    plt.title('vecteur tangentiels et normaux')
    #plt.axis('scaled', adjustable='box')
    if plt_show: plt.show()
...
def aerodynamic_coefficients(panels,c=1.0,alpha=0.0):
    """
    calcul des coefficients aérodynamiques en fonction du Kp sur chaque panneau
    """
...
def aerodynamics(xe,xi,Kpe,Kpi,plt_show=False):
    ...
    DeltaKp=kpi-kpe
    plt.figure()
    plt.xlabel('x/c', fontsize=16);plt.ylabel(r'$\Delta K_p$', fontsize=16)
    ...
    print('Cm0 = ',Cm, ', x centre de poussée = ',Cm/Cl)

    width = 10;ech=0.05
    plt.figure(figsize=(width, width))
    plt.xlabel('x', fontsize=16);plt.ylabel('Kp', fontsize=16)
    plt.title('Coefficient de pression')
    plt.plot(xe,Kpe,linestyle='-', linewidth=1, marker='o', markersize=8, color='red',label='extrados')
    plt.plot(xi,Kpi,linestyle='-', linewidth=1, marker='o', markersize=8, color='blue',label='intrados')
    plt.plot(x,kpe,linestyle='-',color='black',label='extrados')
    plt.plot(x,kpi,linestyle='--',color='black',label='intrados')
