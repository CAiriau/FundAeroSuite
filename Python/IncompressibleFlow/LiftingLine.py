...
#*******************************************
    ...
    if l_0 =0, one between S,b and lamb must equal to zero
    else :
        set l_0 and b
    ...
    prm["wing"]     = "elliptic" # wing shape or lift distribution
...
class WingAnalysis(object):
    ...
    def __init__(self, prm, **kwargs):
        ...
        self.alpha0_law = prm["alpha0_law"]
                                        # alpha_0 law for wing section  "constante", "linear", "other"
        ...
        method:
            1: matrix inversion : collocation  method and inversion or minimization
            2: numerical integration with sine series 
        """
    ...
    def run_analysis(self):
        ...
        self.elliptic_wing()            # reference values for elliptic wing
    ...
    def wing_solve_An(self):
        ...
        if self.method==2:
            ...
            if self.nFourier==int(self.ny-2) : 
                ...
                #self.An,res,rank,s = np.linalg.lstsq(Mat,b_theta[u]*sint[u])
                self.An,res,rank,s = np.linalg.lstsq(Mat,b_theta[u])
                print("res  = ",res)
            ...
            raise("bad choice of method to solve An")
    ...
    def wing_aerodynamics(self):
        ...
        CD =  self.CL**2/(self.lamb*np.pi) + np.pi*self.lamb *np.sum(np.arange(2,self.nFourier+1)*self.An[np.arange(1,self.nFourier)]**2)
    ...
    def elliptic_wing(self):
        ...
        print("-"*80) 
        print("EQUIVALENT ELLIPTIC WING :")
        print("A1 elliptic (analytic)           : %2.6e "%(self.A1_elliptic))
        print("alpha_i     (analytic)           : %2.6e "%(np.rad2deg(-self.A1_elliptic)))
        print("CL elliptic (analytic)           : %2.4f "%(self.CL_elliptic))
        print("CD elliptic (analytic)           : %1.7f "%(self.CD_elliptic))
        print("f  elliptic (analytic)           : %2.8f "%(self.f_elliptic))
        print("lambda/(2+lambda) (analytic)     : %2.6f "%(self.lamb/(2+self.lamb)))
        print("Gamma0 elliptic   (analytic)     : %2.6f "%(self.Gamma0_elliptic))
        print("-"*80) 
    ...
    def wing_plantform(self):
        ...
        if self.wing == "elliptic":
            """ Elliptic wing """
            print("="*5 +"Elliptic wing "+"="*5)
            ...
            self.meanchord = self.S_num/self.span

            print("S num                            : %4.4f m^2"%(self.S_num))
            print("c_mean num                       : %2.4f m"%(self.meanchord))
        ...
        if self.wing=="tapered":
            ...
            if self.span*self.l_0 != 0:
                print("l_0 and b known")
                self.chordtip = self.l_0-tmp*self.b  
                self.S=self.b*(self.l_0+self.chordtip)
                self.lamb=self.span**2/self.S
            ...
            self.chordtip = self.l_0-tmp*self.b  #chord tip
            ...
            self.meanchord=self.S/self.span
            self.S_num=np.trapz(self.chord,x=self.y)

            print("tip chord                        : %4.7f m"%(self.chordtip))
            print("root chord                       : %4.7f m"%(self.l_0))
            print("tip/root chord                   : %4.4f "%(self.chordtip/self.l_0))
            print("lambda num                       : %2.4f "%(self.lamb_num))
            print("S num, S an.                     : %4.4f m^2 , %4.4f m^2"%(self.S_num,self.S))
            print("mean chord num                   : %4.4f m"%(self.meanchord))
            print("mean chord num                   : %4.4f m"%((self.l_0+self.chordtip)/2))
